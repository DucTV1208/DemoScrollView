...
//    var curPage: Int = 0
    ...
    override func viewDidLoad() {
        ...
        timer = Timer.scheduledTimer(timeInterval: 5, target: self, selector: "autoPageControl", userInfo: nil, repeats: true)
    }
    
    @objc func autoPageControl(){
        let width = view.bounds.width
        print("ContentOffset: X: \(scrollView.contentOffset.x) - Y: \(scrollView.contentOffset.y)")
        if pageControl.currentPage < pageControl.numberOfPages-1 {
            scrollView.setContentOffset(CGPoint(x: CGFloat(pageControl.currentPage + 1)*width, y: 0.0), animated: true)
        }
    }
...
//        }
        ...
        let percentOfOne = 1/CGFloat(fruitViews.count-1)
        for i in 0 ..< fruitViews.count-1 {
            if (percentOffset.x > CGFloat(i)*percentOfOne && percentOffset.x <= CGFloat(i+1)*percentOfOne) {
                fruitViews[i].imageView.transform = CGAffineTransform(scaleX: (CGFloat(i+1)*percentOfOne-percentOffset.x)/percentOfOne, y: (CGFloat(i+1)*percentOfOne-percentOffset.x)/percentOfOne)
                fruitViews[i+1].imageView.transform = CGAffineTransform(scaleX: percentOffset.x/(CGFloat(i+1)*percentOfOne), y: percentOffset.x/(CGFloat(i+1)*percentOfOne))
            }
        }
    }
}
